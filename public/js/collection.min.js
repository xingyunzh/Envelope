...
$("a",card).attr("href","/envelope/api/card/view/id/"+collect.card._id);$(".collect-create-date",card).text(stamp(new Date(collect.createDate)))})}).fail(function(error){console.log("Server Error:"+JSON.stringify(error))})}function stamp(date){function format(number){return number<10?"0"+number:""+number}if(date)return""+(date.getMonth()+1)+"-"+date.getDate()+" "+format(date.getHours())+":"+format(date.getMinutes());return""};
